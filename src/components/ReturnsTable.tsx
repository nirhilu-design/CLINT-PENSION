...
// clearinghouse XML) side by side with official treasury data, matched by any
// of the policy's fund codes (product or investment-track level).
export default function ReturnsTable({
  ...
  const rows = policies
    ...
    .map((p) => ({
      policy: p,
      fund: treasuryFunds.find((f) => policyFundCodes(p).includes(f.mofid)),
    }))
          ...
          {rows.map(({ policy, fund }) => {
            ...
            return (
              <tr key={policy.policyNumber} className="border-t border-slate-100">
                ...
                <td className="p-3 tabular text-slate-600">{fund?.mofid ?? policy.mofid ?? '—'}</td>
