...
export default function ClearinghouseRequest() {
  ...
  return (
    <div className="clint-rise text-right rounded-2xl bg-white border border-slate-200/70 shadow-sm overflow-hidden" style={{ animationDelay: '140ms' }}>
      {/* Demo banner */}
      ...
      <div className="p-5">
        ...
        {phase === 'form' && (
          <div className="mt-5 flex flex-col gap-4">
            ...
            <button
              onClick={send}
              disabled={!canSend}
              className="clint-press w-full rounded-xl bg-gradient-to-l from-brand-800 to-brand-700 text-white font-semibold py-3.5 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              שליחת בקשה למסלקה
            </button>
          </div>
        ...
        {phase === 'sending' && (
          <div className="mt-6 flex flex-col items-center text-center py-6 gap-3">
            <span className="w-11 h-11 rounded-full border-2 border-brand-50 border-t-brand-700 animate-spin" />
