...
// Compact fixture mirroring the clearinghouse structure. All values are synthetic
// placeholders — never real client data.
function fixture({ clientId = '100000009', clientId2 = '', sugMutzar = '2' } = {}) {
...
<HeshbonotOPolisot><HeshbonOPolisa>
  ...
  <KIDOD-ACHID>000000000000000000077770000000</KIDOD-ACHID>
  ...
  <PirteiHaasaka><SACHAR-POLISA>12000.00</SACHAR-POLISA></PirteiHaasaka>
  ...
  <PirteiHafkadaAchrona><PerutPirteiHafkadaAchrona><TAARICH-HAFKADA-ACHARON>202409</TAARICH-HAFKADA-ACHARON><TOTAL-HAFKADA>3000.00</TOTAL-HAFKADA></PerutPirteiHafkadaAchrona></PirteiHafkadaAchrona>
  ...
  <Kisuim><ZihuiKisui><KisuiBKerenPensia>
    <SACH-PENSIAT-NECHUT>8000.00</SACH-PENSIAT-NECHUT><SHEUR-KISUY-NECHUT>75.00</SHEUR-KISUY-NECHUT>
    <KITZBAT-SHEERIM-LEALMAN-O-ALMANA>6000.00</KITZBAT-SHEERIM-LEALMAN-O-ALMANA>
    <KITZBAT-SHEERIM-LEYATOM>4000.00</KITZBAT-SHEERIM-LEYATOM>
...
describe('parsePensionXml', () => {
  ...
  it('reads client and product core', () => {
    expect(client.id).toBe('100000009')
  ...
  it('aggregates monthly deposits across contribution types', () => {
    ...
    expect(p.lastDepositTotal).toBe(3000)
  })

  it('reads expected pension and contributions', () => {
    // בהמשך הפקדות מול ללא הפקדות — שני שדות נפרדים בדיווח
    expect(p.expectedPensionWithDeposits).toBe(9000)
    expect(p.expectedPensionWithoutDeposits).toBe(5000)
    expect(p.expectedAccumulationWithDeposits).toBe(2000000)
    expect(p.expectedAccumulationWithoutDeposits).toBe(900000)
  ...
  it('rejects different IDs in one file, accepts padded variants of the same ID', () => {
    ...
    expect(() =>
      parsePensionXml(fixture({ clientId2: '0000000100000009' }), 'padded.xml'),
    ).not.toThrow()
  })
